...
def v5_conn():
    ...
    from memory_cli.db.migrations.v001_baseline_all_tables_indexes_triggers import apply as apply_v001
    from memory_cli.db.migrations.v004_add_access_tracking import apply as apply_v004
    from memory_cli.db.migrations.v005_add_edge_provenance import apply as apply_v005
    from memory_cli.db.migrations.v006_add_consolidated_column import apply as apply_v006
...
class TestV005Migration:
    ...
    def test_migration_adds_consolidated_column(self, v4_conn):
        ...
        from memory_cli.db.migrations.v006_add_consolidated_column import apply as apply_v005
    ...
    def test_runner_migrates_v5_to_v6(self, v4_conn):
        """Migration runner can migrate from v5 to v6 (add consolidated)."""
        ...
        # First bring v4_conn up to v5 (edge provenance)
        result = run_pending_migrations(v4_conn, 4, 5)
        assert result is True
        assert read_schema_version(v4_conn) == 5
        # Now migrate v5 -> v6 (consolidated column)
        result = run_pending_migrations(v4_conn, 5, 6)
        assert result is True
        assert read_schema_version(v4_conn) == 6

    def test_runner_migrates_v0_to_v6(self):
        """Migration runner can migrate from v0 to v6 (full fresh setup)."""
        from memory_cli.db.connection_setup_wal_fk_busy import open_connection
        from memory_cli.db.extension_loader_sqlite_vec import load_and_verify_extensions
        from memory_cli.db.schema_version_reader import read_schema_version
        from memory_cli.db.migration_runner_single_transaction import run_pending_migrations
        ...
        conn = open_connection(":memory:")
        load_and_verify_extensions(conn)
        result = run_pending_migrations(conn, 0, 6)
        assert result is True
